  ...
  try {
    ...
    LEFT JOIN 
        (
            ...
            GROUP BY 
                post_id
        ) scrap_count ON s.post_id = scrap_count.post_id WHERE name ILIKE ${
          "%" + query + "%"
        } OR address ILIKE ${"%" + query + "%"}
     OR explain ILIKE ${"%" + query + "%"}
     OR menu ILIKE ${"%" + query + "%"}
    ORDER BY scrap_count DESC`;
      ...
      FROM 
          katsu_info s
      LEFT JOIN 
          scraps us ON s.post_id = us.post_id AND us.user_email = ${userEmail} WHERE name ILIKE ${
        "%" + query + "%"
      } OR address ILIKE ${"%" + query + "%"}
     OR explain ILIKE ${"%" + query + "%"}
     OR menu ILIKE ${"%" + query + "%"}
